...
- [x] **외부 바이너리 실행 경로** — `binary.run` 툴 도입. `DockerCommandSpec.mounts?` + `RunOptions.mounts?` 추가해 `skillInstallRoot` 를 컨테이너에 read-only bind-mount (`/skills`). interpreter 화이트리스트 (`python3` / `node` / `bash` / `sh`), 경로 traversal 가드, exec-bit 검사, default 이미지 `python:3.12-slim`. 시나리오 두 가지 모두 커버: (A) 컨테이너 이미지에 이미 있는 바이너리 (`/usr/bin/ffmpeg`), (B) skill.create 로 작성한 스크립트/바이너리 (`/skills/<id>/runner.py`). `enableBinaryRun: true` opt-in 필요. 실 Docker 통합 테스트 (`binary-run-tool.docker.integration.test.ts`, `AGENT_DOCKER_INTEGRATION=1` 게이트) 7 케이스: python 스크립트 실행 / baked-in binary / read-only mount 검증 / 직접 exec / 네트워크 차단 / timeout / traversal 거부
- [ ] **DockerSandbox gVisor 런타임 실 검증** — 코드 옵션은 있으나 실 환경에서 `runsc` 동작·성능 미측정. binary.run 통합 테스트와 같은 패턴 (env 게이트) 으로 재사용 가능
