...
```bash
pnpm -r run build    # 19개 패키지 TypeScript 컴파일 (16 top-level + 5 channels)
npx vitest run       # ~760 테스트 (packages/*/src/**/*.test.ts + packages/*/test/**/*.spec.ts)
```

끝에 `Tests <N> passed` 가 보이면 베이스라인이 건강합니다.
...
2. 기본 세션 자동 생성
3. Anthropic Claude Sonnet (`claude-sonnet-4-20250514`, SDK 0.88) 에게 메시지 전송
