...
- **[reference/cli.md](reference/cli.md)** — `agent send / status / ego / gateway / tui / trace / device` 전체 사용법
- **[reference/contracts.md](reference/contracts.md)** — `@agent-platform/core/contracts` 16개 인터페이스 API 레퍼런스
...
A. 가능합니다. 메인 에이전트용 `OpenAIAdapter` 가 구현되어 있고 (`packages/agent-worker/src/model/openai.ts`, native `response_format: 'json_object'` 지원), `AGENT_PROVIDER=openai` 또는 `AGENT_MODEL` 을 `claude-` 로 시작하지 않는 값으로 설정하면 자동 선택됩니다. EGO LLM 도 `ego.json.llm.provider` / `fallback` 에 OpenAI 지정 가능. 엠베더는 `openAIEmbedder` 가 이미 제공됩니다.
...
A. `npx vitest run packages/core` 로 특정 패키지만, 또는 `packages/channels` 만 분리 실행하세요. 전체 ~760개도 30초 내외입니다.
...
| 테스트 일부 실패               | `pnpm install` 재실행 (가끔 node_modules 깨짐)                        |
