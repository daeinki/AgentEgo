...
This code repo has a **sibling design-document repo** at `/mnt/d/ai/claude/` whose docs are the authoritative spec for intent and trade-offs. Code in `packages/` is authoritative for _current behavior_; the specs are authoritative for _why it was done this way_. When a change crosses a public contract (channel adapters, gateway routes, RPC method set, phase event schema, storage paths, auth model, EGO vocabulary), update the matching section of the spec in the same change.
